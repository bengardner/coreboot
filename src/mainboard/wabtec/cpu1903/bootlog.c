...
#define BOOTLOG_REGION     "RW_BOOT_LOG"
...
struct bootlog_region
{
	void   *base;
	size_t size;
};

static int bootlog_get_region(struct bootlog_region *blr)
{
	struct region_device rdev;

	if (fmap_locate_area_as_rdev(BOOTLOG_REGION, &rdev) == 0) {
		blr->size = region_device_sz(&rdev);
		blr->base = rdev_mmap_full(&rdev);
		if (blr->base != NULL) {
			printk(BIOS_DEBUG, "%s: found FMAP '%s' @ %p size=0x%x\n",
			       __func__, BOOTLOG_REGION, blr->base, (uint32_t)blr->size);
			return 0;
		}
	}

	/* find the file storage - if missing, this is a config error */
	blr->base = cbfs_boot_map_with_leak(BOOTLOG_FILENAME, CBFS_TYPE_RAW, &blr->size);
	if (blr->base != NULL) {
		printk(BIOS_DEBUG, "%s: found CBFS '%s' @ %p size=0x%x\n",
		       __func__, BOOTLOG_FILENAME, blr->base, (uint32_t)blr->size);
		return 0;
	}

	printk(BIOS_ERR, "%s: missing FMAP '%s' and CBFS '%s'\n", __func__,
	       BOOTLOG_REGION, BOOTLOG_FILENAME);
	return -1;
}
...
static uint32_t bootlog_find_blank(uint32_t *prev_cnt)
{
	...
	file_ptr   = blr.base;
	file_bytes = blr.size;
	file_recs  = blr.size / sizeof(bootlog_record_t);
