...
{
	static char tmp[128];
	int         hw_rev = inb(CPU1900_REG_HW_REV) & 0x0f;
	int         len;
...
{
	static char parts[64];

	if (!is_print(parts[0])) {
		if (cpu1900_eeprom.valid & CEV_TOPPART) {
			snprintf(parts, sizeof(parts), "%s-%s",
			         cpu1900_eeprom.part_top, cpu1900_eeprom.part_board);
		} else {
			snprintf(parts, sizeof(parts), "%s", cpu1900_eeprom.part_board);
		}
	}
	return parts;
}
...
static void mainboard_enable(device_t dev)
{
	...
	rec.reason = inb(CPU1900_REG_RESET_CAUSE) & 0x0f;
	rec.fpga_slot_id = inb(CPU1900_REG_SLOTID);
	rec.fpga_options = inb(CPU1900_REG_FPGA_OPTIONS);
	bootlog_store(&rec);

	printk(BIOS_INFO, "CPU1900: DTE LED green\n");
	outb(CPU1900_LED_GREEN, CPU1900_REG_DTE_LED_DUTY);
}
